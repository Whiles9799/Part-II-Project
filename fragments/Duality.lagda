...
lemma (CV-sum K₁-V K₂-V) (CV-sum L₁-V L₂-V) refl = cong₂ CV-sum (lemma K₁-V L₁-V refl) (lemma K₂-V L₂-V refl)
lemma CV-not CV-not refl = refl

coval-eq : ∀{Γ Θ A}{K L : Coterm Γ Θ A}(K-V : Covalue K)(L-V : Covalue L)
  → (e : K ≡ L) → ⟨ K , K-V ⟩ ≡ ⟨ L , L-V ⟩
coval-eq K-V L-V e = Inverse.f Σ-≡,≡↔≡ ⟨ e , lemma K-V L-V e ⟩
...
dual-sub-TV-weaken-lemma (Γ , C) Γ′ Θ′ A σ `Z = coval-eq 
...
dual-sub-TV-fmap-lemma (Γ , C) Γ′ Θ′ A σ `Z = coval-eq 
...
dual-sub-C-id-lemma Γ (Θ , B) A (`S x) = trans (dual-sub-C-weaken-lemma Γ Θ Θ B `_ x) (cong (ren-T (λ x₁ → `S x₁) (λ x₁ → x₁)) (dual-sub-C-id-lemma Γ Θ A x))
...
S⟶ⱽT⇒Sᵒ⟶ᴺTᵒ {Γ}{Θ} (V ● μγ {Γ}{Θ}{A} S) .(sub-S TVK CK (add (λ Γ A → Σ (Γ ⟶ _ ∣ A) Value) ⟨ V , v ⟩ (λ x → ⟨ ` x , V-var ⟩)) (λ x → ` x) S) (βL v) = 
  subst (λ - → μθ (S ᵒˢ) ● V ᵒᴸ ˢ⟶ᴺ -) 
    ...
    (βR (Vᵒ≡P V v))
S⟶ⱽT⇒Sᵒ⟶ᴺTᵒ {Γ}{Θ} (μθ {Γ}{Θ}{A} S ● K) .(sub-S TVK CK (λ x → ⟨ ` x , V-var ⟩) (add (λ Θ A → A ∣ _ ⟶ Θ) K (λ x → ` x)) S) βR = 
