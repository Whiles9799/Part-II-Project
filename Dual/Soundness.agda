...
weaken-ren-int-lemma {Γ , B}{Γ′}{A} ρ γ k = cong (λ - → ⟨ - , (ρ `Z ⱽⱽ) γ ⟩) (weaken-ren-int-lemma {Γ}{Γ′}{A} (ren-skip ρ) γ k)


lift-ren-int-lemma : ∀ {Γ Γ′ A} (ρ : Γ ↝ Γ′) γ k → ren-int (Γ , A) (Γ′ , A) (rename-lift ρ) ⟨ γ , k ⟩ ≡ ⟨ (ren-int Γ Γ′ ρ γ) , k ⟩
lift-ren-int-lemma {∅} ρ γ k = refl
lift-ren-int-lemma {Γ , x} ρ γ k = cong (λ - → ⟨ ⟨ - , (ρ `Z ⱽⱽ) γ ⟩ , k ⟩) (weaken-ren-int-lemma (ren-skip ρ) γ k)
...
weaken-neg-ren-int-lemma {Θ , B}{Θ′}{A} ρ θ k = cong (λ - → ⟨ - , (Γ∋A⇒¬Γ∋¬A (ρ `Z) ⱽⱽ) θ ⟩) (weaken-neg-ren-int-lemma (ren-skip ρ) θ k)
...
lift-neg-ren-int-lemma {Θ , x} ρ θ k = cong (λ - → ⟨ ⟨ - , (Γ∋A⇒¬Γ∋¬A (ρ `Z) ⱽⱽ) θ ⟩ , k ⟩) (weaken-neg-ren-int-lemma (ren-skip ρ) θ k)
...
ren-lemma-covar : ∀ {Θ Θ′ A} (α : Θ ∋ A) (ρ : Θ ↝ Θ′) (θ : `¬ˣ Θ′ ⱽˣ) (k : A ⱽᵀ)
  → (Γ∋A⇒¬Γ∋¬A (ρ α) ⱽⱽ) θ k ≡ (Γ∋A⇒¬Γ∋¬A α ⱽⱽ) (neg-ren-int Θ Θ′ ρ θ) k
...
ren-lemma-term : ∀ {Γ Γ′ Θ Θ′ A} (M : Γ ⟶ Θ ∣ A) (s : Γ ↝ Γ′) (t : Θ ↝ Θ′) (c₁ : Γ′ ⱽˣ) (c₂ : `¬ˣ Θ′ ⱽˣ) (k : ((`¬ A) ⱽᵀ))
  → (rename-term s t M ⱽᴸ) ⟨ c₁ , c₂ ⟩ k ≡ (M ⱽᴸ) ⟨ ren-int Γ Γ′ s c₁ , neg-ren-int Θ Θ′ t c₂ ⟩ k 
ren-lemma-termvalue : ∀ {Γ Γ′ Θ Θ′ A} (V : TermValue Γ Θ A) (s : Γ ↝ Γ′) (t : Θ ↝ Θ′) (c₁ : Γ′ ⱽˣ) (c₂ : `¬ˣ Θ′ ⱽˣ)
  → (⟨ rename-term s t (proj₁ V) , value-invariant-under-renaming s t (proj₂ V) ⟩ ⱽᴸⱽ) ⟨ c₁ , c₂ ⟩ ≡ (V ⱽᴸⱽ) ⟨ ren-int Γ Γ′ s c₁ , neg-ren-int Θ Θ′ t c₂ ⟩
ren-lemma-coterm : ∀ {Γ Γ′ Θ Θ′ A} (K : A ∣ Γ ⟶ Θ) (s : Γ ↝ Γ′) (t : Θ ↝ Θ′) (c₁ : Γ′ ⱽˣ) (c₂ : `¬ˣ Θ′ ⱽˣ) (k : A ⱽᵀ)
  → (rename-coterm s t K ⱽᴿ) ⟨ c₁ , c₂ ⟩ k ≡ (K ⱽᴿ) ⟨ ren-int Γ Γ′ s c₁ , neg-ren-int Θ Θ′ t c₂ ⟩ k    
ren-lemma-statement : ∀ {Γ Γ′ Θ Θ′} (S : Γ ↦ Θ) (s : Γ ↝ Γ′) (t : Θ ↝ Θ′) (c₁ : Γ′ ⱽˣ) (c₂ : `¬ˣ Θ′ ⱽˣ)
  → (rename-statement s t S ⱽˢ) ⟨ c₁ , c₂ ⟩ ≡ (S ⱽˢ) ⟨ ren-int Γ Γ′ s c₁ , neg-ren-int Θ Θ′ t c₂ ⟩
...
ren-lemma-term {Γ}{Γ′}{Θ}{Θ′}{A} (μθ S) s t c₁ c₂ k =
  ...
  ≡⟨ cong (λ - → (S ⱽˢ) ⟨ ren-int Γ Γ′ s c₁ , - ⟩) (lift-neg-ren-int-lemma t c₂ k) ⟩
...
ren-lemma-coterm {Γ}{Γ′}{Θ}{Θ′}{A} `[ K , L ] s t c₁ c₂ k = cong (λ - → - k) {(rename-coterm s t `[ K , L ] ⱽᴿ) ⟨ c₁ , c₂ ⟩ }{(`[ K , L ] ⱽᴿ) ⟨ ren-int Γ Γ′ s c₁ , neg-ren-int Θ Θ′ t c₂ ⟩} (ext (λ{(inj₁ x) → ren-lemma-coterm K s t c₁ c₂ x ; (inj₂ y) → ren-lemma-coterm L s t c₁ c₂ y}))
...
ren-lemma-coterm {Γ}{Γ′}{Θ}{Θ′}{A} (μγ S) s t c₁ c₂ k = 
  ...
  ≡⟨ cong (λ - → (S ⱽˢ) ⟨ - , neg-ren-int Θ Θ′ t c₂ ⟩) (lift-ren-int-lemma s c₁ k) ⟩
...
termvalue-sub-weaken-lemma {Γ}{Γ′}{Θ}{A}{B} σ `Z γ θ k = 
  ...
  ≡⟨ ren-lemma-termvalue (σ `Z) (λ x → `S x) (λ x → x) ⟨ γ , k ⟩ θ ⟩
    ((σ `Z) ⱽᴸⱽ) ⟨ ren-int Γ′ (Γ′ , B) (λ x → `S x) ⟨ γ , k ⟩ , neg-ren-int Θ Θ (λ x → x) θ ⟩
  ≡⟨ cong₂ (λ -₁ -₂ → (σ `Z ⱽᴸⱽ) ⟨ -₁ , -₂ ⟩) (trans (weaken-ren-int-lemma (λ x → x) γ k) (id-ren γ)) (id-neg-ren θ) ⟩
    (σ `Z ⱽᴸⱽ) ⟨ γ , θ ⟩
  ∎
...
termvalue-sub-fmap-lemma {Γ}{Γ′}{Θ}{A}{B} σ `Z γ θ k = 
  ...
  ≡⟨ ren-lemma-termvalue (σ `Z) (λ x → x) (λ x → `S x) γ ⟨ θ , k ⟩ ⟩
    (σ `Z ⱽᴸⱽ) ⟨ ren-int Γ′ Γ′ (λ x → x) γ , neg-ren-int Θ (Θ , B) (λ x → `S x) ⟨ θ , k ⟩ ⟩
  ≡⟨ cong₂ (λ -₁ -₂ → (σ `Z ⱽᴸⱽ) ⟨ -₁ , -₂ ⟩) (id-ren γ) (trans (weaken-neg-ren-int-lemma (λ x → x) θ k) (id-neg-ren θ)) ⟩
    (σ `Z ⱽᴸⱽ) ⟨ γ , θ ⟩ 
  ∎
...
lift-termvalue-sub-int-lemma : ∀ {Γ Γ′ Θ A} (σ : Γ –[(λ Γ A → TermValue Γ Θ A)]→ Γ′) γ θ k
  → termvalue-sub-int (Γ , A) (Γ′ , A) Θ (sub-lift (TermSubstKit.kit TermValueKit) σ) θ ⟨ γ , k ⟩ ≡ ⟨ (termvalue-sub-int Γ Γ′ Θ σ θ γ) , k ⟩
...
fmap-termvalue-sub-int-lemma : ∀ {Γ Γ′ Θ A} (σ : Γ –[(λ Γ A → TermValue Γ Θ A)]→ Γ′) γ θ k
  → termvalue-sub-int Γ Γ′ (Θ , A) (fmap {λ Γ B → TermValue Γ Θ B} {λ Γ B → TermValue Γ (Θ , A) B} (TermSubstKit.wkΘ TermValueKit) σ) ⟨ θ , k ⟩ γ ≡ termvalue-sub-int Γ Γ′ Θ σ θ γ
...
coterm-sub-weaken-lemma {Γ}{Θ}{Θ′}{A}{B} σ `Z γ θ k = ext (λ z → 
  ...
  ≡⟨ ren-lemma-coterm (σ `Z) (λ x → x) (λ x → `S x) γ ⟨ θ , k ⟩ z ⟩
    (σ `Z ⱽᴿ) ⟨ ren-int Γ Γ (λ x → x) γ , neg-ren-int Θ′ (Θ′ , B) (λ x → `S x) ⟨ θ , k ⟩ ⟩ z
  ≡⟨ cong₂ (λ -₁ -₂ → (σ `Z ⱽᴿ) ⟨ -₁ , -₂ ⟩ z) (id-ren γ) (trans (weaken-neg-ren-int-lemma (λ x → x) θ k) (id-neg-ren θ)) ⟩
    ((σ `Z ⱽᴿ) ⟨ γ , θ ⟩ z) 
  ∎)
...
coterm-sub-fmap-lemma : ∀ {Γ Θ Θ′ A B} (σ : Θ –[ (λ Θ A → A ∣ Γ ⟶ Θ) ]→ Θ′) (α : Θ ∋ A) γ θ k
  → (fmap {λ Θ A → A ∣ Γ ⟶ Θ} {λ Θ A → A ∣ Γ , B ⟶ Θ} (CotermSubstKit.wkΓ CotermKit) σ α ⱽᴿ) ⟨ ⟨ γ , k ⟩ , θ ⟩ ≡ (σ α ⱽᴿ) ⟨ γ , θ ⟩
coterm-sub-fmap-lemma {Γ}{Θ}{Θ′}{A}{B} σ `Z γ θ k = ext (λ z → 
  ...
  ≡⟨ ren-lemma-coterm (σ `Z) (λ x → `S x) (λ x → x) ⟨ γ , k ⟩ θ z ⟩
    (σ `Z ⱽᴿ) ⟨ ren-int Γ (Γ , B) (λ x → `S x) ⟨ γ , k ⟩ , neg-ren-int Θ′ Θ′ (λ x → x) θ ⟩ z
  ≡⟨ cong₂ (λ -₁ -₂ → (σ `Z ⱽᴿ) ⟨ -₁ , -₂ ⟩ z) (trans (weaken-ren-int-lemma (λ x → x) γ k) (id-ren γ)) (id-neg-ren θ) ⟩
    (σ `Z ⱽᴿ) ⟨ γ , θ ⟩ z
  ∎)
...
lift-coterm-sub-int-lemma : ∀ {Γ Θ Θ′ A} (σ : Θ –[ (λ Θ A → A ∣ Γ ⟶ Θ) ]→ Θ′) γ θ k
  → coterm-sub-int Γ (Θ , A) (Θ′ , A) (sub-lift (CotermSubstKit.kit CotermKit) σ) γ ⟨ θ , k ⟩ ≡ ⟨ coterm-sub-int Γ Θ Θ′ σ γ θ , k ⟩
...
fmap-coterm-sub-int-lemma : ∀ {Γ Θ Θ′ A} (σ : Θ –[ (λ Θ A → A ∣ Γ ⟶ Θ) ]→ Θ′) γ θ k
  → coterm-sub-int (Γ , A) Θ Θ′ (fmap {λ Θ B → B ∣ Γ ⟶ Θ} {λ Θ B → B ∣ (Γ , A) ⟶ Θ} (CotermSubstKit.wkΓ CotermKit) σ) ⟨ γ , k ⟩ θ ≡ coterm-sub-int Γ Θ Θ′ σ γ θ
...
sub-lemma-term {Γ}{Γ′}{Θ}{Θ′}{A} s t (μθ S) γ θ = ext (λ k → 
  begin
    ...
    (fmap {λ Γ B → TermValue Γ Θ′ B} {λ Γ B → TermValue Γ (Θ′ , A) B} (TermSubstKit.wkΘ TermValueKit) s)
    (sub-lift (CotermSubstKit.kit CotermKit) t)
    S ⱽˢ) ⟨ γ , ⟨ θ , k ⟩ ⟩
  ≡⟨ sub-lemma-statement (fmap {λ Γ B → TermValue Γ Θ′ B} {λ Γ B → TermValue Γ (Θ′ , A) B} (TermSubstKit.wkΘ TermValueKit) s) (sub-lift (CotermSubstKit.kit CotermKit) t) S γ ⟨ θ , k ⟩ ⟩
    (S ⱽˢ) 
    ⟨ termvalue-sub-int Γ Γ′ (Θ′ , A) (fmap {λ Γ B → TermValue Γ Θ′ B} {λ Γ B → TermValue Γ (Θ′ , A) B} (TermSubstKit.wkΘ TermValueKit) s) ⟨ θ , k ⟩ γ , 
    ⟨ coterm-sub-int Γ′ Θ (Θ′ , A) (sub-weaken (CotermSubstKit.kit CotermKit) t) γ ⟨ θ , k ⟩ , k ⟩ ⟩
  ≡⟨ cong (λ - → (S ⱽˢ) ⟨ termvalue-sub-int Γ Γ′ (Θ′ , A) (fmap {λ Γ B → TermValue Γ Θ′ B} {λ Γ B → TermValue Γ (Θ′ , A) B} (TermSubstKit.wkΘ TermValueKit) s) ⟨ θ , k ⟩ γ , ⟨ - , k ⟩ ⟩) (weaken-coterm-sub-int-lemma t γ θ k) ⟩
    (S ⱽˢ) 
    ⟨ termvalue-sub-int Γ Γ′ (Θ′ , A) (fmap {λ Γ B → TermValue Γ Θ′ B} {λ Γ B → TermValue Γ (Θ′ , A) B} (TermSubstKit.wkΘ TermValueKit) s) ⟨ θ , k ⟩ γ ,     
...
sub-lemma-coterm {Γ}{Γ′}{Θ}{Θ′}{A} s t (μγ S) γ θ = ext (λ x → 
  begin 
    ...
    (fmap {λ Θ B → B ∣ Γ′ ⟶ Θ} {λ Θ B → B ∣ Γ′ , A ⟶ Θ} (CotermSubstKit.wkΓ CotermKit) t)
    S ⱽˢ) ⟨ ⟨ γ , x ⟩ , θ ⟩
  ≡⟨ sub-lemma-statement (sub-lift (TermSubstKit.kit TermValueKit) s) (fmap {λ Θ B → B ∣ Γ′ ⟶ Θ} {λ Θ B → B ∣ Γ′ , A ⟶ Θ} (CotermSubstKit.wkΓ CotermKit) t) S ⟨ γ , x ⟩ θ ⟩
    (S ⱽˢ)
    ⟨ ⟨ termvalue-sub-int Γ (Γ′ , A) Θ′ (sub-weaken (TermSubstKit.kit TermValueKit) s) θ ⟨ γ , x ⟩ , x ⟩ ,
    coterm-sub-int (Γ′ , A) Θ Θ′ (fmap {λ Θ B → B ∣ Γ′ ⟶ Θ} {λ Θ B → B ∣ Γ′ , A ⟶ Θ} (CotermSubstKit.wkΓ CotermKit) t) ⟨ γ , x ⟩ θ ⟩
  ≡⟨ cong (λ - → (S ⱽˢ) ⟨ ⟨ - , x ⟩ , coterm-sub-int (Γ′ , A) Θ Θ′ (fmap {λ Θ B → B ∣ Γ′ ⟶ Θ} {λ Θ B → B ∣ Γ′ , A ⟶ Θ} (CotermSubstKit.wkΓ CotermKit) t) ⟨ γ , x ⟩ θ ⟩) (weaken-termvalue-sub-int-lemma s γ θ x) ⟩
    (S ⱽˢ)
