...
sub-lemma-statement : ∀ {T C Γ Γ′ Θ Θ′ A} (s : Γ –[ (λ - → T - Θ′) ]→ Γ′) (t : Θ –[ (C Γ′) ]→ Θ′) (S : Γ ↦ Θ) (c : Γ ⱽˣ × `¬ˣ Θ ⱽˣ) → ((sub-statement TermKit CotermKit s t S) ⱽˢ) c ≡ (((S ⱽˢ)) ∘ λ x → ⟨ ((s ⱽᵘ) (proj₁ x)) , ((t ⱽⁿ) (proj₂ x)) ⟩) c

sub-lemma-statement s t (M ● K) = {!   !}
...
S⟶ⱽT⇒Sⱽ≡Tⱽ (V ● μγ S) .(S ⟨ V /⟩ˢ) ⟨ c₁ , c₂ ⟩ (βL v) = sym (
  begin
    ((S ⟨ V /⟩ˢ) ⱽˢ) ⟨ c₁ , c₂ ⟩
  ≡⟨⟩
    (sub-statement TermKit CotermKit (λ x → (λ{`Z → V ; (`S x) → ` x}) x) id-coterm S ⱽˢ) ⟨ c₁ , c₂ ⟩
  ≡⟨ {!   !} ⟩
...
S⟶ⱽT⇒Sⱽ≡Tⱽ (μθ S ● K) .(S [ K /]ˢ) ⟨ c₁ , c₂ ⟩ (βR) = sym (
  begin
    ((S [ K /]ˢ) ⱽˢ) ⟨ c₁ , c₂ ⟩
  ≡⟨⟩
    (sub-statement TermKit CotermKit id-term (λ x → (λ{`Z → K ; (`S x) → ` x}) x) S ⱽˢ) ⟨ c₁ , c₂ ⟩
  ≡⟨ sub-lemma-statement {!   !} {!   !} S ⟨ {!   !} , ⟨ {!   !} , (λ x → {!  !}) ⟩ ⟩ ⟩
    (S ⱽˢ) ⟨ c₁ , ⟨ c₂ , (K ⱽᴿ) ⟨ c₁ , c₂ ⟩ ⟩ ⟩
  ∎)
S—↠ⱽT⇒Sⱽ≡Tⱽ : ∀ {Γ Θ} (S T : Γ ↦ Θ) (c : (Γ ᵒˣ) ᴺˣ × `¬ˣ (Θ ᵒˣ) ᴺˣ) → S ˢ—↠ⱽ T → (S ⱽˢ) c ≡ (T ⱽˢ) c
